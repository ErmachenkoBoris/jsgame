...
function draw(){
        ...
        if(!!stop){
            ...
            for(let i=0; i<pipe.length; i++) {
                ...
                if(((xPos + ball.width-5)>=pipe[i].x && yPos>(pipe[i].y+WallSize) && (xPos<pipe[i].x))  || yPos>(cvs.height-15)) {
                    
                    restart();
                    break;
                }
                if((xPos + ball.width-5)>=pipe[i].x && yPos<(pipe[i].y+WallSize) && !pipe[i].score ) {
                    pipe[i].score=1;
                    score++;
                }
